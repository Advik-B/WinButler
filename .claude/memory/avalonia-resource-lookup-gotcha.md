...
(2026-07-18 update: `ThemeService.Apply` and the whole red/green swap were deleted in the v7 UI pass — the app is green-only and the `WbAccent*` keys hold green directly. The lesson still applies to `ThemeService.Brush`, which is the surviving C#-side lookup and does it correctly.)
